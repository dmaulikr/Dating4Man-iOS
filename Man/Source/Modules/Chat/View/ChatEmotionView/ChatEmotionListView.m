...
@interface ChatEmotionListView()<UIScrollViewDelegate, ChatEmotionCreditsCollectionViewCellDelegate>
...
+ (instancetype)emotionListView:(id)owner {
    NSArray *nibs = [[NSBundle mainBundle] loadNibNamedWithFamily:@"ChatEmotionListView" owner:owner options:nil];
    ChatEmotionListView* view = [nibs objectAtIndex:0];

    UINib *nib = [UINib nibWithNibName:@"ChatEmotionCreditsCollectionViewCell" bundle:nil];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ChatHighGradeEmotion *item = [self.largeEmotions objectAtIndex:indexPath.item];
    cell.tag = indexPath.item;
    if( item.image ) {
        cell.largeEmotionImageView.image = item.image;
    } else {
        cell.largeEmotionImageView.image = [UIImage imageNamed:@"Chat-LargeEmotionDefault"];
    }
