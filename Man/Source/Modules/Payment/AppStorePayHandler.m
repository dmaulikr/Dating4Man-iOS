...
@implementation AppStorePayHandler
#pragma mark - 初始化
/**
 *  初始化
 *
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
    ...
    if ([response.products count] > 0)
    {
        ...
        if (nil != product) {
            ...
            myPayment.requestData = [AppStorePayHandler getRequestData:self.manId orderNo:self.orderNo];
...
+ (NSString* _Nonnull)getOrderNo:(SKPaymentTransaction* _Nonnull)transaction
{
    ...
    NSArray* strings = [requestStr componentsSeparatedByString:[AppStorePayHandler getRequestDataSeparator]];
...
+ (NSData* _Nonnull)getRequestData:(NSString* _Nonnull)manId orderNo:(NSString* _Nonnull)orderNo
{
    ...
    requestStr = [NSString stringWithFormat:@"%@%@%@", manId, [AppStorePayHandler getRequestDataSeparator], orderNo];
