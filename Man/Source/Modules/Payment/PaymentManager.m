...
@interface PaymentManager () <PaymentHandlerDelegate, LoginManagerDelegate>
// delegate
@property (nonatomic, strong) NSMutableArray<NSValue*>* delegates;
// 支付处理器 dictionary
@property (nonatomic, strong) NSMutableDictionary<NSString*, PaymentHandler*>* handlers;
// 待重支付处理器 array
@property (nonatomic, strong) NSMutableArray<PaymentHandler*>* autoRetryHandlers;
 ...
 *  @return this
 */
+ (PaymentManager* _Nonnull)manager
{
...
{
    // 退出LoginManager回调
    [[LoginManager manager] removeDelegate:self];
...
- (BOOL)pay:(NSString* _Nonnull)number
{
        ...
        && (nil != self.sid && [self.sid length] > 0))
    {
        ...
        // 获取订单号请求
        SessionRequestManager* sessionRequestMgr = [SessionRequestManager manager];
        if (nil != sessionRequestMgr) {
            GetPaymentOrderRequest* request = [[GetPaymentOrderRequest alloc] init];
            ...
            request.finishHandler = ^(BOOL success, NSString * _Nonnull code, NSString * _Nonnull orderNo, NSString * _Nonnull productId)
            {
                ...
                if (success && [orderNo length] > 0) {
                    PaymentHandler* handler = [[PaymentHandler alloc] initWithOrderNo:orderNo productId:productId manId:blockManId sid:blockSid delegate:self];
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray<SKPaymentTransaction *> *)transactions
{
    for (SKPaymentTransaction* tran in transactions)
    {
        // 查找支付处理器
        PaymentHandler* handler = nil;
        NSString* orderNo = [AppStorePayHandler getOrderNo:tran];
        if (nil != orderNo && [orderNo length] > 0) {
            @synchronized (self.handlers) {
                ...
                // 若支付处理器不存在
                if (nil == handler) {
                    NSString* manId = [AppStorePayHandler getManId:tran];
                    if (nil != manId && [self.manId isEqualToString:manId]) {
                        // 当前男士与AppStore支付信息的男士一致
                        handler = [[PaymentHandler alloc] initWithOrderNo:orderNo productId:tran.payment.productIdentifier manId:manId sid:self.sid delegate:self];
                        [self.handlers setObject:handler forKey:orderNo];
                    }
                }
            }
        ...
        if (nil != handler) {
            ...
            {
                [AppStorePayHandler finish:tran];
            }
        }
    }
}
 ...
 */
- (void)updatePaymentState:(PaymentHandler* _Nonnull)handler state:(PaymentHandleState)state success:(BOOL)success code:(NSString* _Nonnull)code isNetErr:(BOOL)isNetErr
{
    // 是否可retry
...
- (void)timerAutoRetryPay:(NSTimer*)timer
{
    ...
    @synchronized (self.autoRetryHandlers)
    {
        for (PaymentHandler* handler in self.autoRetryHandlers) {
            [handler start];
        }
    }
}
