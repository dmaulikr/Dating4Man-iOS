...
- (BOOL)addDelegate:(id<LiveChatManagerDelegate> _Nonnull)delegate
{
    ...
    {
        // 查找是否已存在
        for (id<LiveChatManagerDelegate> temp in self.delegates)
        {
            if (temp == delegate) {
                NSLog(@"LiveChatManager::addDelegate() add again, delegate:<%@,%p>", delegate, delegate);
                result = YES;
                break;
            }
        }
...
- (BOOL)removeDelegate:(id<LiveChatManagerDelegate> _Nonnull)delegate
{
    ...
    // log
    if (!result) {
        NSLog(@"LiveChatManager::removeDelegate() fail, delegate:<%@,%p>", delegate, delegate);
    }
     ...
     receVideoMsg:(bool)isRecvVideoMsg
{
    ...
    if (NULL != mILiveChatManManager)
    {
        ...
        result = result && mILiveChatManManager->Login([user UTF8String], [userSid UTF8String], HttpClient::GetCookiesInfo(), [[[UIDevice currentDevice] model] UTF8String], isRecvVideoMsg);
    }
    return result;
}
        ...
        isAutoLogin:(bool)isAutoLogin
{
    ...
    @synchronized(self.delegates) {
        for (id<LiveChatManagerDelegate> delegate in self.delegates)
        {
 ...
 *  @return 操作是否成功
 */
- (BOOL)getUserStatus:(NSArray* _Nonnull)userIds
{
    BOOL result = NO;
    if (NULL != mILiveChatManManager)
    {
...
{
    NSString* nssErrMsg = [NSString stringWithUTF8String:errMsg];
    NSArray* users = [LiveChatItem2OCObj getLiveChatUserArray:userList];
    @synchronized (self.delegates)
    {
        for (id<LiveChatManagerDelegate> delegate in self.delegates)
        {
            if( [delegate respondsToSelector:@selector(onGetUserStatus:errMsg:userList:)] ) {
 ...
 *  @return 操作是否成功
 */
- (BOOL)getUsersInfo:(NSArray* _Nonnull)userIds
{
    BOOL result = NO;
    if (NULL != mILiveChatManManager) {
...
{
    NSString* nsErrMsg = [NSString stringWithUTF8String:errMsg];
    NSArray* usersInfo = [LiveChatItem2OCObj getLiveChatUserInfoArray:userInfoList];
    @synchronized(self.delegates)
    {
        for (id<LiveChatManagerDelegate> delegate in self.delegates)
        {
            if( [delegate respondsToSelector:@selector(onGetUsersInfo:errMsg:userInfoList:)] ) {
 ...
 *  @return 返回
 */
- (NSArray* _Nonnull)getMsgsWithUser:(NSString* _Nonnull)userId
{
 ...
 *  @return 用户数组
 */
- (NSArray* _Nonnull)getInviteUsers
{
 ...
 *  @return 用户数组
 */
- (NSArray* _Nonnull)getChatingUsers
{
    NSArray* users = [NSArray array];
    if (NULL != mILiveChatManManager)
    {
...
- (void)onSendTextMsgsFail:(LCC_ERR_TYPE)errType msgList:(const LCMessageList&)msgList
{
    NSArray* msgs = [LiveChatItem2OCObj getLiveChatMsgArray:msgList];
    @synchronized(self.delegates)
    {
        for (id<LiveChatManagerDelegate> delegate in self.delegates)
        {
...
- (void)onGetUsersHistoryMessage:(bool)success errNo:(const string&)errNo errMsg:(const string&) errMsg userList:(const LCUserList&)userList
{
    NSArray* nsUserIds = [LiveChatItem2OCObj getLiveChatUserIdArray:userList];
    NSString* nsErrNo = [NSString stringWithUTF8String:errNo.c_str()];
    NSString* nsErrMsg = [NSString stringWithUTF8String:errMsg.c_str()];
    @synchronized(self.delegates)
    {
